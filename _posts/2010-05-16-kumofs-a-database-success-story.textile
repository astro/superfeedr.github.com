...
Well, you can't have everything: when Riak thrashed the whole machine due to throughput, KumoFS fares much better, but the kumo-gateway process was slowly growing its request buffer. Having this process eating swap space before we can add machines to our cluster will be fatal. Our safety belt: *ulimit -v*
...
KumoFS has many knobs to turn, and we still have to reattach a restarted server to the cluster. But so far, it is making us so happy that we are seriously beginning to consider it for replacement of our "Redis":http://code.google.com/p/redis/ cluster. We are annoyed that Redis requires so much RAM, and client-side sharding is cumbersome.
