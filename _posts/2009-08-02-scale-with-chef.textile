...
There are several options for the configuration. The most basic one is to _do everything by hand_ (and try to repeat for as many servers you need to deploy). Another one is to _deal with an "image"_ (a ghost if you want), but then it's quite hard to improve it and evolve it. At superfeedr, the option we chose is to use "Chef":http://wiki.opscode.com/display/chef/Home.
